...
同一只宠物若同时装在 `~/.petdex` 和 `~/.codex`,菜单里会去重为一条。内置**形象**(Blob、Stacky、Byte、Glitch)完全不需要素材 —— 它们是程序化绘制并动画的,所有状态开箱即动。
...
到 [最新 Release](https://github.com/JellyTony/codepet/releases/latest) 下载 `CodePet-macos.zip`,解压后双击 **`Install CodePet.command`**(若 macOS 拦截:右键 → 打开 → 打开)。它会把 CodePet 复制到 `/Applications`、清除下载隔离、接好 Claude Code 的 hooks 并启动宠物。需要 macOS 13+ 和 [Node.js](https://nodejs.org)(hooks 用)。
